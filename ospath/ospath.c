...
// 设置函数的完成状态
static inline void osp_stat(int state) { OSP_LAST_STATE = state; }

// 获取最后一次函数执行的错误代码
// 返回0代表没有发生错误
// 此函数返回值见同名头文件中名称以<STATUS_>开头的宏定义
int path_last_error(void) { return OSP_LAST_STATE; }
...
// 参数buf是接收路径的缓冲区
// 参数size是缓冲区大小
// 如果参数buf为NULL，则应注意，之前保存get_cwd(NULL, 0)返回值的(char
// *)变量的值都有可能被此次运行改变
...
scanlist_t *make_scanlist(size_t blocks) {
    scanlist_t *scanlst;
    osp_stat(STATUS_EXEC_SUCCESS);
    if (blocks <= 0) blocks = MALLOC_NUM;
    scanlst = malloc(blocks * sizeof(scanlist_t) + sizeof(char *));
    if (!scanlst) {
        osp_stat(STATUS_MEMORY_ERROR);
        return NULL;
    }
...
void delete_scanlist(scanlist_t *scanlst) {
    ...
    if (scanlst) {
        for (size_t i = 0; i < scanlst->count; ++i) {
            if (scanlst->paths[i]) free(scanlst->paths[i]);
        }
...
int enrich_scanlist(const char *dir_path, int p_type, int subdirs,
    ...
    scanlist_t *tmp_p_scanlst;  // 仅为了让Visual Studio不显示警告
    char *buf_full_path = NULL;
    char buffer_find_path[PATH_MSIZE];
    int ret_value = RESULT_SUCCESS;
    osp_stat(STATUS_EXEC_SUCCESS);
    if (NULL == pp_scanlst || NULL == *pp_scanlst) {
        osp_stat(STATUS_EMPTY_POINTER);
        return RESULT_FAILURE;
    }
    if (!is_dir(dir_path)) {
        if (!path_last_error()) osp_stat(STATUS_INVALID_PARAM);
        return RESULT_FAILURE;
    }
    tmp_p_scanlst = *pp_scanlst;
    ...
    if (join_path(buffer_find_path, PATH_MSIZE, 2, buffer_find_path, OSP_AFS))
        return RESULT_FAILURE;
    ...
    while (0 != FindNextFileA(hfind, &find_paths)) {
        if (!strcmp(find_paths.cFileName, PATH_CDIRS) ||
            ...
            continue;
        if ((*pp_scanlst)->count >= (*pp_scanlst)->blocks) {
            // 用sizeof(*pp_scanlst)得不到原对象已分配内存大小
            tmp_p_scanlst = realloc(
                *pp_scanlst,
                sizeof(scanlist_t) +
                    sizeof(char *) * ((*pp_scanlst)->blocks + RALLOC_NUM));
            if (NULL == tmp_p_scanlst) {
                ret_value = RESULT_FAILURE;
                osp_stat(STATUS_MEMORY_ERROR);
                goto close_return;
            }
            *pp_scanlst = tmp_p_scanlst;
            (*pp_scanlst)->blocks += RALLOC_NUM;
        }
        ...
        if (join_path(buf_full_path, len_bytes_malloc, 2, dir_path,
                      find_paths.cFileName)) {
            free(buf_full_path);
            continue;
        }
        if (!normal_path(buf_full_path, len_bytes_malloc)) {
            free(buf_full_path);
            continue;
        }
        if (find_paths.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (p_type & PTYPE_BOTH || p_type & PTYPE_DIR)
                (*pp_scanlst)->paths[(*pp_scanlst)->count++] = buf_full_path;
            if (subdirs)
                ret_value = enrich_scanlist(buf_full_path, p_type, subdirs,
                                            pp_scanlst);
            if (!(p_type & PTYPE_BOTH) && !(p_type & PTYPE_DIR))
                if (NULL != buf_full_path)
                    free(buf_full_path), buf_full_path = NULL;
            if (ret_value == RESULT_FAILURE) goto close_return;
        } else {
            if (p_type & PTYPE_BOTH || p_type & PTYPE_FILE)
                (*pp_scanlst)->paths[(*pp_scanlst)->count++] = buf_full_path;
...
int enrich_scanlist(const char *dir_path, int p_type, int subdirs,
    ...
    DIR *p_dopened;
    p_dopened = opendir(dir_path);
    while (NULL != (p_dent = readdir(p_dopened))) {
...
bool is_abs(const char *_path) {
    ...
    strcpy(p_tmp, _path);
    if (!normal_case(p_tmp))
        return RESULT_FALSE;
    else if (!strncmp(p_tmp, PATH_UNCQS, 4)) {
        osp_stat(STATUS_OTHER_ERRORS);
        return RESULT_TRUE;
    } else if (split_drv(NULL, 0, p_tmp, PATH_MSIZE, p_tmp))
        return RESULT_FALSE;
    return (strlen(p_tmp) > 0 && *p_tmp == PATH_NSEP);
}
...
char *normal_path(char path[], size_t size) {
    ...
    if (split_drv(prefix, PATH_MSIZE, suffix, PATH_MSIZE, path)) {
        result_s = NULL;
        goto clean_return;
    }
    ...
    if (!*prefix && index == 0) {
        ...
        for (size_t i = 0; i < index; ++i) {
            ...
            strcat(p_tmp, splited[i]);
            if (i != index - 1) strcat(p_tmp, PATH_NSEPS);
        }
        strcpy(path, p_tmp);
    }
clean_return:
...
//      例如 split_drv(buf_d, 10, buf_p, 10, "c:\\dir")，buf_d被设置为 "c:"，
//      buf_p被设置为"\\dir"
//
//      如果路径包含共享点路径，则buf_d将包含主机名并共享最多但不包括
//      第四个目录分隔符。例如 split_drv(buf_d, 10, buf_p, 10,
//      "//host/computer/dir")
...
int split_drv(char buf_d[], size_t bdsize, char buf_p[], size_t bpsize,
    ...
    char p_tmp[PATH_MSIZE], cased[PATH_MSIZE];
    osp_stat(STATUS_EXEC_SUCCESS);
    if (!_path) {
        osp_stat(STATUS_EMPTY_POINTER);
        return RESULT_FAILURE;
    ...
    if (p_len >= 2) {
        strcpy(cased, p_tmp);
        if (!normal_case(cased)) return RESULT_FAILURE;
        if (!strncmp(cased, PATH_NSEPS2, 2) && cased[2] != PATH_NSEP) {
            ...
            if (NULL != buf_d) {
                ...
                strncpy(buf_d, p_tmp, sep4_index - cased);
            ...
            if (NULL != buf_p) {
                ...
                strcpy(buf_p, p_tmp + (sep4_index - cased));
            }
            return RESULT_SUCCESS;
        }
        if (p_tmp[1] == PATH_COLON) {
            if (NULL != buf_d) {
                if (bdsize < 3) {
                    osp_stat(STATUS_INSFC_BUFFER);
                    return RESULT_FAILURE;
                }
                strncpy(buf_d, p_tmp, 2);
                buf_d[2] = EMPTY_CHAR;
            }
            if (NULL != buf_p) {
                if (bpsize < p_len - 1) {
...
int join_path(char buf[], size_t bfsize, int n, const char *_path, ...) {
    ...
    va_list arg_list;
    if (split_drv(res_drv, PATH_MSIZE, res_pth, PATH_MSIZE, _path)) {
        ret_value = RESULT_FAILURE;
        goto clean_return;
    }
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        if (split_drv(p_drv, PATH_MSIZE, p_pth, PATH_MSIZE, arg_string)) {
            ret_value = RESULT_FAILURE;
            goto clean_return;
        }
...
int split_path(char buf_h[], size_t bhsize, char buf_t[], size_t btsize,
               const char *_path) {
    int index, ret_value = RESULT_SUCCESS;
    ...
    if (split_drv(p_drv, PATH_MSIZE, p_pth, PATH_MSIZE, _path))
        return RESULT_FAILURE;
...
char *dir_name(char buf_dir[], size_t bfsize, const char *_path) {
    ...
    return split_path(buf_dir, bfsize, NULL, 0, _path) ? NULL : buf_dir;
}
...
char *base_name(char buf_base[], size_t bfsize, const char *_path) {
    ...
    return split_path(NULL, 0, buf_base, bfsize, _path) ? NULL : buf_base;
}

// 功能：生成相对路径
// 此相对路径是_path相对于start的路径
// 成功返回0，失败返回1
int rel_path(char buf[], size_t size, const char *_path, const char *start) {
    ...
    if (abs_path(p_abs, PATH_MSIZE, _path)) goto clean_return;
    if (abs_path(s_abs, PATH_MSIZE, start)) goto clean_return;
    if (split_drv(buf_p, PATH_MSIZE, p_abs, PATH_MSIZE, p_abs))
        goto clean_return;
    if (split_drv(buf_s, PATH_MSIZE, s_abs, PATH_MSIZE, s_abs))
        goto clean_return;
...
// 生成给定路径的绝对路径
// 成功返回0，失败返回1
int abs_path(char buf[], size_t size, const char *_path) {
    size_t length;
    osp_stat(STATUS_EXEC_SUCCESS);
    if (!_path) {
        osp_stat(STATUS_EMPTY_POINTER);
    ...
    return (normal_path(buf, size)) ? RESULT_SUCCESS : RESULT_FAILURE;
}
...
int split_path(char buf_h[], size_t bhsize, char buf_t[], size_t btsize,
    ...
    strcpy(p_tmp, _path);
    p_len = last_sep_plus1 = strlen(p_tmp);
    while (last_sep_plus1 && p_tmp[last_sep_plus1 - 1] != PATH_NSEP)
        --last_sep_plus1;
    if (last_sep_plus1 > 0) strncpy(head, p_tmp, last_sep_plus1);
    ...
    if (NULL != buf_t) {
        ...
        strcpy(buf_t, p_tmp + last_sep_plus1);
    }
    return RESULT_SUCCESS;
}
...
int rel_path(char buf[], size_t bfsize, const char *_path, const char *start) {
    ...
    p_abs = malloc(PATH_MSIZE);
    s_abs = malloc(PATH_MSIZE);
    p_tmp = malloc(PATH_MSIZE);
    if (!p_abs || !s_abs || !p_tmp) {
        ret_value = RESULT_FAILURE;
        osp_stat(STATUS_MEMORY_ERROR);
        goto clean_return;
    }
    if (!start) start = PATH_CDIRS;
    if (abs_path(p_abs, PATH_MSIZE, _path)) {
        ret_value = RESULT_FAILURE;
        goto clean_return;
    }
    if (abs_path(s_abs, PATH_MSIZE, start)) {
        ret_value = RESULT_FAILURE;
        goto clean_return;
    }
    if (strlen(p_abs) <= 0)
    ...
    for (int i = 0; i < (cnt_s - cnt_same); ++i) {
        ...
        strcat(p_tmp, PATH_PDIRS);
        strcat(p_tmp, PATH_NSEPS);
    }
    for (int i = cnt_same; i < cnt_p; ++i) {
...
int abs_path(char buf[], size_t bfsize, const char *_path) {
    char p_tmp[PATH_MSIZE] = {0};
    char *end_ch = p_tmp;
    if (!is_abs(_path)) {
        if (path_last_error()) return RESULT_FAILURE;
        if (!get_cwd(p_tmp, PATH_MSIZE)) return RESULT_FAILURE;
    }
    if (strlen(p_tmp) + strlen(_path) >= PATH_MSIZE) {
        osp_stat(STATUS_PATH_TOO_LONG);
        return RESULT_FAILURE;
    }
    while (*end_ch) ++end_ch;
    if (end_ch != p_tmp) --end_ch;
    if (*end_ch != PATH_NSEP && *_path != PATH_NSEP) {
        if (strlen(p_tmp) + 1 >= PATH_MSIZE) {
            osp_stat(STATUS_PATH_TOO_LONG);
            return RESULT_FAILURE;
        }
        strcat(p_tmp, PATH_NSEPS);
    ...
    strcat(p_tmp, _path);
    if (!normal_path(p_tmp, PATH_MSIZE)) return RESULT_FAILURE;
    if (bfsize <= strlen(p_tmp)) {
        osp_stat(STATUS_INSFC_BUFFER);
        return RESULT_FAILURE;
    }
...
int split_ext(char buf_h[], size_t bhsize, char buf_e[], size_t besize,
    ...
    strcpy(p_tmp, _path);
    nsep_index = strrchr(p_tmp, PATH_NSEP);
#ifdef _WIN32
    asep_index = strrchr(p_tmp, PATH_ASEP);
    if (asep_index > nsep_index) nsep_index = asep_index;
#endif
    if (!extsep) extsep = PATH_ESEP;
    dot_index = strrchr(p_tmp, extsep);
    if (dot_index > nsep_index) {
        ...
        while (name_index < dot_index) {
            if (*name_index != extsep) {
                dot_index_n = dot_index - p_tmp;
                if (NULL != buf_h) {
                    if (bhsize <= dot_index_n) {
                        osp_stat(STATUS_INSFC_BUFFER);
                        return RESULT_FAILURE;
                    }
                    strncpy(buf_h, p_tmp, dot_index_n);
...
int prune_path(char buf[], size_t bfsize, const char *_path) {
    ...
    p_tmp = malloc(PATH_MSIZE * sizeof(char));
    p_tmp2 = malloc(PATH_MSIZE * sizeof(char));
    splited = malloc(PATH_MSIZE * sizeof(char *));
    if (!p_tmp || !p_tmp2 || !splited) {
        ret_value = RESULT_FAILURE;
        osp_stat(STATUS_MEMORY_ERROR);
        goto clean_return;
    }
    strcpy(p_tmp, _path);
    if (!normal_path(p_tmp, PATH_MSIZE)) {
        ret_value = RESULT_FAILURE;
        goto clean_return;
    }
    stk = strtok(p_tmp, PATH_NSEPS);
    while (stk) {
        if (count > PATH_MSIZE) {
            ret_value = RESULT_FAILURE;
            osp_stat(STATUS_PATH_TOO_LONG);
